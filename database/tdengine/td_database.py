...
class TDengineDatabase(BaseDatabase):
    ...
    def __init__(self) -> None:
        ...
        self.symbol_tick_dict: dict = defaultdict(list)
        self.symbol_bar_dict: dict = defaultdict(list)
        self.queue = Queue()
        self.thread = Thread(target=self.run)
        self.active = False
        self.database_type = 'time_sequence'
        self.start()
        ...
        while self.active:
            try:
                ...
                if isinstance(task, tuple):
                    task_type, data = task
                    if task_type == "tick":
                        self.symbol_tick_dict[data.symbol].append(data)
                        if len(self.symbol_tick_dict[data.symbol]) >= 120:
                            self.save_tick_data_from_dict(self.symbol_tick_dict[data.symbol], stream=True)
                            self.symbol_tick_dict[data.symbol] = []
                    elif task_type == "bar":
                        self.symbol_bar_dict[data.symbol].append(data)
                        if len(self.symbol_bar_dict[data.symbol]) >= 2:
                            self.save_bar_data_from_dict(self.symbol_bar_dict[data.symbol], stream=True)
                            self.symbol_bar_dict[data.symbol] = []
                elif task is None:
                    self.save_tick_data_symbol_batch(self.symbol_tick_dict)
                    self.save_bar_data_symbol_batch(self.symbol_bar_dict)
                ...
                self.active = False
                print(sys.exc_info())
                print(f"错误模块: {e.__class__.__module__}.{e.__class__.__name__}")

    def save_bar_data_symbol_batch(self, symbol_dict: dict):
    ...
    def save_bar_data_from_dict(self, bars: List[BarData], stream: bool = False) -> bool:
        ...
        # 写入k线数据
        self.insert_in_batch(table_name, bars, 2000)

        # 查询汇总信息
        self.cursor.execute(f"show table tags FROM {table_name}")
        results: List[tuple] = self.cursor.fetchall()

        overview: tuple = results[0]
        overview_start: datetime = overview[4]
        overview_end: datetime = overview[5]
        overview_count: int = int(overview[6])
        ...
        print(f'bar 数据完成,{symbol},{overview_count}')

        return True

    def save_bar_data(self, data: dict, stream: bool = False) -> bool:
        self.queue.put(data)
        return True

    def save_tick_data(self, data: dict, stream: bool = False) -> bool:
        self.queue.put(data)
        return True

    def save_tick_data_from_dict(self, ticks: List[TickData], stream: bool = False) -> bool:
        """保存tick数据"""
        ...
        # 写入tick数据
        self.insert_in_batch(table_name, ticks, 2000)
        # 查询汇总信息
        self.cursor.execute(f"show table tags  FROM {table_name}")
        results: List[tuple] = self.cursor.fetchall()

        overview: tuple = results[0]
        overview_start: datetime = overview[3]
        overview_end: datetime = overview[4]
        overview_count: int = int(overview[5])
        ...
        self.cursor.execute(f"ALTER TABLE {table_name} SET TAG end_time='{overview_end}';")
        self.cursor.execute(f"ALTER TABLE {table_name} SET TAG count_='{overview_count}';")
        print(f'tick 数据完成,{symbol},{overview_count}')

        return True

    def update_tag_count_info(self, table_name: str):
        self.cursor.execute(f"select count(*) from {table_name}")
        results: List[tuple] = self.cursor.fetchall()
        ...
        if bar_count != overview_count:
            self.cursor.execute(f"ALTER TABLE {table_name} SET TAG count_='{bar_count}';")
            print(f"{table_name}的数据条数更新完成，前值:{overview_count}, 现值:{bar_count}")
        else:
            print(f"{table_name}的数据条数一致，无需更新")
